...
def _main(args):
    ...
    if args.obs_file:
        ...
        title_b = '(b) Maximum TXx from model ensemble'
    else:
        fig = plt.figure(figsize=[10, 8])
        ax2 = fig.add_subplot(111)
        title_b = 'Record temperature distribution for one thousand climate model sub-samples'
    
    # Panel b: Model data
    ds_ensemble = fileio.open_dataset(args.ensemble_file, units={'tasmax': args.units})
    ...
    logging.info(f'Maximum model TXx: {maximum} {args.units}')
    ...
    #ax2.set_ylabel(f'TXx ({args.units})')
    ax2.set_ylabel(f'record temperature ({args.units})')

    # Panel a: Obs
    if args.obs_file:
        ds_obs = fileio.open_dataset(args.obs_file, units={'tasmax': args.units})
        ...
        ax1.set_ylabel(f'TXx ({args.units})')   
    ...
    plt.savefig(args.outfile, metadata={metadata_key: new_log}, bbox_inches='tight', facecolor='white', dpi=300)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--obs_file", type=str, default=None, help="Observations TXx file")
    parser.add_argument(
        '--units',
        type=str,
        default='degC',
